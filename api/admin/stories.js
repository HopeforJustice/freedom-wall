    ...
    switch (req.method) {
      ...
      case 'POST':
        ...
        if (!lock_id || !title || !body) {
          return res.status(400).json({ error: 'lock_id, title, and body are required' });
        }
      ...
      case 'DELETE':
        ...
        return res.status(200).json({ message: 'Story deleted successfully', story: deletedStory[0] });
